...
from web.services import pages, settings_api, settings_public, settings_save_endpoint, ui_lang
...
async def api_settings_public_route():
    return settings_api.get_settings_public(settings_public.public_settings_payload, load_yaml_config())
...
async def api_settings_save_route(request: Request):
    ...
    out = await settings_save_endpoint.api_save_settings(
        ...
        sync_cursor_proxy=lambda cfg: asyncio.to_thread(cursor_proxy.sync_cursor_proxy_from_config, cfg),
    )
    return out
